...
class NeuralNetwork:
    ...
    def save(self, name=None):
        ...
        if not name:
            ...
            np.save('gen_data/' + name + '_weights', self.weights)
            np.save('gen_data/' + name + '_biases', self.biases)
    ...
    def render(self, window, vision):
        ...
        for i in range(len(network)):                                           # for each layer
            for j in range(len(network[i])):                                    # for each neuron in current layer
                ...
                pygame.gfxdraw.aacircle(window, x, y, 9, (0, 205, 205))  
                ...
                if (len(network[i]) == 3):
                    ...
                    if (j == 0) :
                        label = myfont.render("FORWARD" , 1, (intensity  , intensity , 20))
                        # put the label object on the screen at point x=100, y=100
                        window.blit(label, (x + 20, y - 10))
                    elif (j == 1) :
                        label = myfont.render("RIGHT" , 1, (intensity  , intensity , 20 ))
                        # put the label object on the screen at point x=100, y=100
                        window.blit(label, (x + 20, y - 10))
                    elif (j == 2) :
                        label = myfont.render("LEFT" , 1, (intensity , intensity , 20 ))
                        # put the label object on the screen at point x=100, y=100
                        window.blit(label, (x + 20, y - 10))
